...
Render a deterministic Markdown command reference from the public registry
without installing files. See
[docs/command-reference-generation.md](docs/command-reference-generation.md)
for the explicit-output workflow and validation command.
...
shell-completion, man-page, command-metadata, and command-reference generation, built-executable smoke checks, and repository hygiene
...
test expansion, documentation polish, or supply-chain policy tightening until
that gate is clean.
...
- [docs/command-reference-generation.md](docs/command-reference-generation.md): deterministic Markdown command reference
...
공개 registry에서 결정적 Markdown 명령 reference를 생성할 수 있으며 파일을 자동 설치하지
않습니다. 명시적 출력 경로와 검증 방법은
[docs/command-reference-generation.md](docs/command-reference-generation.md)를 참고하세요.
...
doctest suite, template-instantiation workflow, shell completion, man page, 명령 metadata와 command reference 생성, 빌드된 실행 파일
...
출력하는 동안에는 다음 write-capable maintenance task가 artifact hygiene cleanup이어야 합니다.
그 gate가 깨끗해질 때까지 우선순위가 낮은 hardening, test expansion, documentation polish,
supply-chain policy tightening에 write-task 주기를 쓰지 마세요.
...
- [docs/command-reference-generation.md](docs/command-reference-generation.md): 결정적 Markdown 명령 reference
