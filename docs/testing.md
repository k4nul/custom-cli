...
is the authoritative validation path until CI is added.
...
Keep generated build trees and local configs out of commits. The repository
already ignores common build directories plus `config/local.json` and
`config/*.local.json`.
